...
def processar_rodada(rodada: int) -> None:
    ...
    if verificar_morte(morto):
        return  # Finaliza a rodada se o jogo terminou.
...
def verificar_morte(morto: int) -> bool:
    ...
    if morto == 0:  # Se o usuário morreu, o jogo acaba
        JOGADORES_VIVOS -= 1
        return user_morto(jogadores[0].player)
    
    elif jogadores[morto].funcao == "mafioso":  # Vitória se o mafioso for morto
        JOGADORES_VIVOS -= 1
        print("O mafioso foi eliminado! A cidade venceu!")
        return True  # True indica fim de jogo
...
def debate() -> bool:
    '''Realiza um debate entre os jogadores e verifica se o mafioso é eliminado.'''
    global JOGADORES_VIVOS
    sleep(2)
    if jogadores[0].vida == 0:
    ...
    if acusacao[1] != -1:
        ...
        if jogadores[acusacao[1]].funcao == "mafioso":  # Vitória se mafioso for eliminado no debate
            print("O mafioso foi eliminado durante o debate! A cidade venceu!")
            return True  # Termina o jogo por vitória
...
def jogador_venceu() -> bool:
    ...
    if jogadores[0].funcao == "mafioso":
        return jogadores[0].vida == 1 and JOGADORES_VIVOS == 2  # Vitória para o mafioso
    else:
        for jogador in jogadores:
            if jogador.funcao == "mafioso" and jogador.vida == 0:  # Vitória se mafioso morto
                return True
    return False
